...
if ($_POST['submitForm2'] == "Respond")
{
    ...
    if ($_POST['decision'] == "accept") {
        ...
        Thank you for submitting objects for review to the Insects Incorporated database of cultural entomology!
...
//        <A HREF="javascript:popUp(\'http://culturalentomology.org/objects\')">objects</A><br /><br />';
  ...
  while($row = mysql_fetch_object($result))
  {
     ...
     // Store filenames
     $fileIDs[$irow] = $ID;
     $filenames[$irow] = $filename1;
  
     // Line
     echo '<hr size="1" />';
     echo '<i>Object ID:</i> '.$ID.'<br />';
     ...
     if (strlen($filename1) > 0) {
         ...
         if ( in_array($extension, $allowedExts ) ) {
             ...
             echo '   <span class="font80">'.$filename1.' ('.$ID.')</span>';
             echo '  </td>';
             echo '  <td width="560">';
         } else {
             echo '<i>File name:</i> '.stripslashes($filename1).'<br />';
         }
     }
